...
```
  ...
  │     ├─ criticAgent.Evaluate → score 0-10           │
  │     └─ score<7 → доп. поиск → повтор               │
  │  6. return ResearchResult{Report, SourceURLs}      │
  └────────────────────────────────────────────────────┘
            │                           ▲
            ▼                           │
  ┌────────────────────┐       ┌──────────────────┐
...
6. criticLoop(ctx, title, sectionReports)
   ...
   │   b. criticAgent.Evaluate → score 0-10
...
**ProgressReporter**: функция `(event string, data map[string]any)`, вызывается на каждом этапе для SSE-трансляции.
...
processPage(url, query):
  ...
  3. summarizeContent(content) → LLM extracts 3-7 key facts
...
**`Evaluate(ctx, report) → (score, feedback, error)`**:

```
LLM prompt: оцени отчёт по критериям:
- полнота раскрытия темы (0-10)
- глубина анализа (0-10)
- качество синтеза (0-10)
→ JSON {"score": 7, "feedback": "...", "details": {...}}
```

Если score >= 7, цикл критика прерывается досрочно. Иначе координатор извлекает gap-запросы из фидбека, выполняет дополнительный поиск и синтезирует новую версию.

---

### 2.6 `internal/agent/plan.go` — типы плана
...
**Методы**: `Create`, `Complete`, `Fail`, `Get`, `List`. `List()` возвращает метаданные без `ReportText`.
...
**Методы**:
- `CompleteSimple(ctx, model, system, prompt, temperature) → (string, error)` — основной метод
- `Complete(ctx, model, messages, temperature) → (string, error)` — с произвольными сообщениями
...
HTTP-запрос
  ...
  └─ GET /api/research/stream → SSE
      └─ ReportStore.Create → (горутина) Coordinator.Run
              ...
              ▼
        ResearchResult{Report, SourceURLs}
              │
              ▼
        ReportStore.Complete(id, text, urls, duration)
```
...
### 5.7 Отчёты с full lifecycle
Report отслеживает полный lifecycle: `in_progress` → `completed`/`error`. Поддерживается polling на странице деталей для незавершённых отчётов.
